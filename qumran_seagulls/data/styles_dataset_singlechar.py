...
class StylesDatasetSinglechar(ABC):
    def __init__(self,
                 root: str,
                 char_label: int,
    ...
    def load_from_folders(self, root: str) -> List[Character]:
        ...
        style_datasets = [[Character(image=s.image, label=s.label, style=i) for s in ds if s.label == self.char_label] for i, ds in enumerate(style_datasets)]
